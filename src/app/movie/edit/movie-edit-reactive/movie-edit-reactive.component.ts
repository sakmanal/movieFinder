...
@Component({
  ...
  styleUrls: ['./movie-edit-reactive.component.css']
})
export class MovieEditReactiveComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.editForm = this.fb.group({
      ...
      director: ['', [Validators.required, Validators.minLength(5), Validators.maxLength(50)]],
  ...
  onValueChanged(data: any): void {
    Object.keys(this.formError).forEach(field => {
      const hasError = this.editForm.get(field).dirty && !this.editForm.get(field).valid;
